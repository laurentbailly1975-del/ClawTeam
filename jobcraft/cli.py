...
def _load_env() -> None:
    """Load .env from ~/.jobcraft/ if present."""
...
def generate(
    ...
    instructions: Optional[str] = typer.Option(
        None, "--instructions", "-i",
        help="Instructions spécifiques (ex: 'insiste sur le management')",
    ),
    open_after: bool = typer.Option(False, "--open", "-o", help="Ouvrir dans le navigateur"),
) -> None:
    ...
    if not job.get("analysis"):
        ...
        job = store.get_job(job_id)  # reload
    ...
    if cv_ok and cover_ok:
        ...
        console.print("[dim]PDF non généré (weasyprint/wkhtmltopdf absent). "
                      "Ouvrez les .html dans un navigateur → Imprimer → Enregistrer en PDF.[/dim]")
...
def tweak(
    ...
    # Re-generate PDF if possible
    pdf_cv = store.docs_path(job_id) / "cv.pdf"
    pdf_cover = store.docs_path(job_id) / "cover.pdf"
    renderer.html_to_pdf(cv_path, pdf_cv)
    renderer.html_to_pdf(cover_path, pdf_cover)
...
def show(
    ...
    """Afficher les détails d'une offre (texte + analyse)."""
    store.init()
    job = store.get_job(job_id)
    if not job:
        console.print(f"[red]Offre #{job_id} introuvable.[/red]")
    ...
    if job.get("analysis"):
        ...
        if a.get("ats_keywords"):
            console.print(f"[bold]Mots-clés ATS :[/bold] {', '.join(a['ats_keywords'])}")
